...
If boundaries for code review if not clear, ask using `AskUserQuestion` tool boundaries like: `Entire project`, `Uncommited changes`, `Last commit`...

Review requested code like you are Rust expert and using following instructions:
...
6. Optimize build speed when relevant:
   ...
   - Use `sccache` to cache compiled artifacts during development in big projects.
